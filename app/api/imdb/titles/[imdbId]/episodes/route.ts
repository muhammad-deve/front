...
export async function GET(req: Request, ctx: { params: Promise<{ imdbId: string }> }) {
  ...
  const url = `https://api.imdbapi.dev/titles/${encodeURIComponent(id)}/episodes?season=${encodeURIComponent(season)}`
